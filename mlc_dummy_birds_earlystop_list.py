...
att = h5py.File('att_vgg_birds_list.h5')
att_test = h5py.File('att_vgg_birds_test_list.h5')
obj = json.load(open('raw_train_birds_list.json','r'))
obj_test = json.load(open('raw_test_birds_list.json','r'))
train = []
test = []
labels = []
no_training_samples = 1300
no_test_samples = 600
...
for i in range(no_training_samples):
  label = np.zeros([10]).astype('int')	
  train.append(att['maps_with_vgg'][i])
  label[obj[i]['bird_ids']] = 1
  labels.append(label)
...
class _classifier(nn.Module):
    def __init__(self, featureDim , nTargets):
        ...
        self.main = nn.Sequential(
        	...
        	#nn.ReLU(),
                nn.Linear(featureDim,165),
                nn.Dropout(0.6),
                nn.ReLU(),
                nn.Linear(165, nTargets),
...
print classifier 
optimizer = optim.Adam(classifier.parameters(), weight_decay = 8e-3)
...
for epoch in range(epochs):
    ...
    print 'Test set performance:', (correct_count*100)/600.0,'%'
...
trained_model = torch.save(classifier,'3_layer_birds.t7')
classifier_test = _classifier(feature_dim,no_classes).cuda()
classifier_test = torch.load('3_layer_birds.t7')
