...
att = h5py.File('att_vgg_vegetation_list.h5')
att_test = h5py.File('att_vgg_vegetation_test_list.h5')
obj = json.load(open('raw_train_vegetation_list.json','r'))
obj_test = json.load(open('raw_test_vegetation_list.json','r'))
train = []
test = []
labels = []
no_training_samples = 3700
no_test_samples = 1900
...
for i in range(no_training_samples):
  label = np.zeros([13]).astype('int')	
  train.append(att['maps_with_vgg'][i])
  label[obj[i]['vegetation_ids']] = 1
  labels.append(label)
...
print classifier 
optimizer = optim.Adam(classifier.parameters(), weight_decay = 2e-3)
...
for epoch in range(epochs):
    ...
    for j in range(no_test_samples):
      ...
      # out = torch.sigmoid(predicted).data > 0.5
       ...
       for o in obj_test[j]['vegetation_ids']:
         if idx[0] == o:
           correct_count += 1
           break
    ...
    print 'Test set performance:', (correct_count*100)/1900.0,'%'
...
trained_model = torch.save(classifier,'3_layer_vegetation.t7')
classifier_test = _classifier(feature_dim,no_classes).cuda()
classifier_test = torch.load('3_layer_vegetation.t7')
