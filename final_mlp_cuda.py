...
features = h5py.File('compo_features_u_train.h5')
...
print vqa
vqa.load_state_dict(torch.load('models_u/3_layer_u_mlp_80.t7'))  
sample_idx = range(no_training_samples - crop)
optimizer = optim.Adam(vqa.parameters(), weight_decay = 1e-4)
...
for epoch in range(epochs):
	...
	for batch_no in range(len(T)):
         with torch.cuda.device(2):
    	   labelsv = Variable(torch.from_numpy(np.array(L[batch_no])).cuda())
    	   inputv = Variable(torch.FloatTensor(torch.from_numpy(np.array(T[batch_no]))).cuda()).view(batch_size, -1)
	...
	if (epoch+1)%10 == 0:
		 torch.save(vqa.state_dict(),'models_u/3_layer_u_mlp_' + str(80 + epoch+1) + '.t7')
